...
impl IcmpEchoRequest {
    ...
    pub fn create_packet<'a>(&self, buffer: &'a mut [u8]) -> Result<MutableEchoRequestPacket<'a>, PingError> {
        // 首先填充payload
        ...
        // 然后创建packet
        let mut packet = MutableEchoRequestPacket::new(buffer).ok_or(PingError::PacketConstructionError)?;
