...
2. 用 `↑/↓` 或 `j/k` 在设备列表中导航
3. 按 `Enter` 进入选中设备的详情页面
4. 在详情页面中按 `↑/↓` 或 `j/k` 滚动查看离线事件
5. 按 `q` 或 `ESC` 返回设备列表
6. 按 `s` 切换排序方式
7. 按 `q` 退出程序
...
启用 UI 模式时，PingUltra 会自动创建 `pingultra_monitor.db` SQLite 数据库，用于保存：

- **所有离线事件**：包括离线时间、恢复时间、持续时长
- **设备统计**：每个设备的离线次数、平均离线时长等
- **历史记录**：保留最近 30 天的数据

### 查看历史数据

详情页面会同时显示：
1. **内存中的事件**：当前会话中发生的离线事件
2. **数据库中的事件**：历史离线事件（最多显示 100 条）

这样你可以看到设备的完整离线历史，即使重启程序也不会丢失数据。
